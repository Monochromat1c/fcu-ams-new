...
@section('content')
<link rel="stylesheet" href="{{ asset('css/asset.css') }}"> {{-- Reuse existing styles if applicable --}}
    ...
    <div :class="{ 'col-span-5': sidebarOpen, 'col-span-6': !sidebarOpen }" class="bg-slate-200 content min-h-screen">
        <nav class="bg-white flex justify-between py-3 px-4 m-3 shadow-md rounded-md">
            ...
            <h1 class="my-auto text-2xl md:text-3xl font-semibold text-gray-800 truncate">
                Turnover History: {{ $decodedAssigneeName }}
            </h1>
            {{-- User Profile Dropdown (Copied from assignedAssets for consistency) --}}
        ...
        <div class="bg-white p-6 shadow-lg m-3 rounded-lg">
            <div class="flex justify-between items-center mb-6">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-xl font-bold text-gray-800">Asset Turnover History</h2>
                </div>
                <a href="{{ route('asset.assigned') }}" class="text-sm text-blue-600 hover:underline flex items-center gap-1">
            ...
            @if($turnoverHistory->isEmpty())
                <div class="text-center py-10">
                    ...
                    <p class="mt-1 text-sm text-gray-500">This assignee has not been involved in any asset turnovers yet.</p>
                </div>
            @else
                            ...
                            @foreach($turnoverHistory as $history)
                                <tr class="hover:bg-gray-50 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <span class="{{ $history->previous_assignee == $decodedAssigneeName ? 'font-bold text-red-600' : '' }}">
                                            {{ $history->previous_assignee }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <span class="{{ $history->new_assignee == $decodedAssigneeName ? 'font-bold text-green-600' : '' }}">
                                            {{ $history->new_assignee }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $history->turnover_date->format('M d, Y g:i A') }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-500 max-w-xs truncate" title="{{ $history->notes }}">{{ $history->notes ?? 'N/A' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
